...
export default function VisitCandPaymentOutDetails() {
  ...
  const deletePaymentIn = async (payment) => {
    if (window.confirm('Are you sure you want to delete this record?')){
      ...
      try {
        const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/visit/candidates/delete/single/payment_out`, {
          method: 'DELETE',
          headers: {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/visit/candidates/update/single/payment_out`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `Bearer ${user.token}`,
  ...
  const handleTotalPaymentUpdate = async () => {
    setLoading3(true)
    try {
      const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/visit/candidates/update/all/payment_out`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `Bearer ${user.token}`,
  ...
  const deleteTotalpayment = async (person) => {
    if (window.confirm('Are you sure you want to delete this record?')){
      setLoading5(true)
      try {
        const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/visit/candidates/delete/all/payment_out`, {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
            "Authorization": `Bearer ${user.token}`,
