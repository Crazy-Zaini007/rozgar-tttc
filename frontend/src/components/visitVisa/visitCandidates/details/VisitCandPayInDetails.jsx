...
export default function VisitCandPaymentInDetails() {
  ...
  const { getVisitCandPaymentsIn } = VisitHook()
  ...
  const deletePaymentIn = async (payment) => {
    if (window.confirm('Are you sure you want to delete this record?')){
      ...
      try {
        const response = await fetch(`${apiUrl}/auth/visit/candidates/delete/single/payment_in`, {
          method: 'DELETE',
          headers: {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await fetch(`${apiUrl}/auth/visit/candidates/update/single/payment_in`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `Bearer ${user.token}`,
  ...
  const handleTotalPaymentUpdate = async () => {
    setLoading3(true)
    try {
      const response = await fetch(`${apiUrl}/auth/visit/candidates/update/all/payment_in`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `Bearer ${user.token}`,
  ...
  const deleteTotalpayment = async (person) => {
    if (window.confirm('Are you sure you want to delete this record?')){
      setLoading5(true)
      try {
        const response = await fetch(`${apiUrl}/auth/visit/candidates/delete/all/payment_in`, {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
            "Authorization": `Bearer ${user.token}`,
  ...
  const filteredTotalPaymentIn = visitCand_Payments_In.filter(payment => {
    return (
  ...
  const printMainTable = () => {
    ...
    if (filteredTotalPaymentIn.length === 1) {
        const { supplierName, contact, pp_No, trade, company, total_Visa_Price_In_PKR,total_Cash_Out, country, flight_Date, total_Payment_In } = filteredTotalPaymentIn[0];
                ...
                <div class="row justify-content-center">
                    ...
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <p>Rozgar Visa Price: <b>${total_Visa_Price_In_PKR}</b></p>
                            <p>Total In: <b>${total_Payment_In}</b></p>
                            <p>Remaining: <b>${total_Visa_Price_In_PKR - total_Payment_In + total_Cash_Out}</b></p>
                        </div>
                    </div>
                </div>
            </div>
          ...
          <div className='col-md-12 '>
            <Paper className='py-3 mb-2 px-2 d-flex justify-content-between'>
              ...
              <div className="right d-flex">
                {visitCand_Payments_In.length > 0 &&
                  <>
              ...
              <div className="row">
                <div className="col-auto px-1">
                  ...
                  <select value={date1} onChange={(e) => setDate1(e.target.value)} className='m-0 p-1'>
                    ...
                    {[...new Set(visitCand_Payments_In.map(data => data.createdAt))].map(dateValue => (
                ...
                <div className="col-auto px-1">
                  ...
                  <select value={name} onChange={(e) => setName(e.target.value)} className='m-0 p-1'>
                    <option value="">All</option>
                    {visitCand_Payments_In && visitCand_Payments_In.map((data) => (
    ...
    {filteredTotalPaymentIn.reduce((total, paymentItem) => {
        ...
        return total + netCashOut;
                ...
                <TablePagination
                  ...
                  component='div'
                  count={visitCand_Payments_In.length}
