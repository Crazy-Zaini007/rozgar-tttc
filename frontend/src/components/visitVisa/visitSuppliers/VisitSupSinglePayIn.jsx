...
export default function VisitSupSinglePayIn() {
  ...
  const handleForm = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch('https://api-rozgar-tttc.onrender.com/auth/visit/suppliers/add/payment_in', {
        method: 'POST',
        headers: {
  ...
  const handleUploadList =async (e) => {
    ...
    e.preventDefault()
    try {
      const response = await fetch('https://api-rozgar-tttc.onrender.com/auth/visit/suppliers/add/multiple/payment_in', {
        method: "POST",
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${user.token}`,
    ...
    <>
      <div className="col-md-12">
        <Paper className='py-3 mb-1 px-2'>
          ...
          <button className='btn m-1 py-2 btn-sm entry_btn' onClick={() => setSingle(0)} style={single === 0 ? { backgroundColor: 'var(--accent-lighter-blue)', color: 'var(--white)', transition: 'background-color 0.3s', transform: '0.3s' } : {}}>Single Payment-In</button>
          <button className='btn m-1 py-2 btn-sm entry_btn' onClick={() => setSingle(1)} style={single === 1 ? { backgroundColor: 'var(--accent-lighter-blue)', color: 'var(--white)', transition: 'background-color 0.3s', transform: '0.3s' } : {}}>Multiple Payment-In</button>
                    ...
                    {visitSupplier_Payments_In
                      ...
                      .map((filteredData) => (
                        ...
                        <TableRow>
                          ...
                          <TableCell className='label border'>Total_VisitVisa_Price_In_PKR</TableCell>
                          <TableCell className=' data_td text-center  bg-info text-white text-bold'>{filteredData.total_Azad_Visa_Price_In_PKR}</TableCell>
                          <TableCell></TableCell>
                          <TableCell></TableCell>
                          <TableCell className='label border'>Total_VisitVisa_Price_In_Curr</TableCell>
