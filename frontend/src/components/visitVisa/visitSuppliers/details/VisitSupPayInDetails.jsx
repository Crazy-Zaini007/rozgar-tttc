...
export default function VisitSupPayInDetails() {
    ...
    const deletePaymentIn = async (payment) => {
        if (window.confirm('Are you sure you want to delete this record?')){
            ...
            try {
                const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/visit/suppliers/delete/single/payment_in`, {
                    method: 'DELETE',
                    headers: {
    ...
    const deletePerson = async (person) => {
        if (window.confirm('Are you sure you want to delete this record?')){
            ...
            try {
                const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/visit/suppliers/delete/person/payment_in`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        "Authorization": `Bearer ${user.token}`,
    ...
    const handleUpdatePerson = async () => {
        setLoading5(true)
        try {
            const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/visit/suppliers/payment_in/update/single/person`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    "Authorization": `Bearer ${user.token}`,
    ...
    const handleUpdate = async () => {
        ...
        try {
            const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/visit/suppliers/update/single/payment_in`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    "Authorization": `Bearer ${user.token}`,
    ...
    const deleteTotalpayment = async (person) => {
        if (window.confirm('Are you sure you want to delete this record?')){
            setLoading5(true)
            try {
                const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/visit/suppliers/delete/all/payment_in`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        "Authorization": `Bearer ${user.token}`,
    ...
    const filteredTotalPaymentIn = visitSupplier_Payments_In.filter(payment => {
        return (
  ...
  `;
    ...
    const filteredPersons = visitSupplier_Payments_In
        .filter((data) => data.supplierName === selectedSupplier)
        .map((filteredData) => ({
            ...filteredData,
                    ...
                    <div className='col-md-12 '>
                        <Paper className='py-3 mb-2 px-2 d-flex justify-content-between'>
                            ...
                            <div className="right d-flex">
                                {visitSupplier_Payments_In.length > 0 &&
                                    <>
                            ...
                            <div className="row">
                                <div className="col-auto px-1">
                                    ...
                                    <select value={date1} onChange={(e) => setDate1(e.target.value)} className='m-0 p-1'>
                                        ...
                                        {[...new Set(visitSupplier_Payments_In.map(data => data.createdAt))].map(dateValue => (
                                ...
                                <div className="col-auto px-1">
                                    ...
                                    <select value={supplier1} onChange={(e) => setSupplier1(e.target.value)} className='m-0 p-1'>
                                        <option value="">All</option>
                                        {visitSupplier_Payments_In && visitSupplier_Payments_In.map((data) => (
                                    ...
                                    <Table stickyHeader>
                                        <TableHead>
                                            ...
                                            <TableRow>
                                                ...
                                                <TableCell className='label border'>TVPriceIn_Oth_Curr</TableCell>
                            ...
                            <div className="row">
                                <div className="col-auto px-1">
                                    ...
                                    <select value={date2} onChange={(e) => setDate2(e.target.value)} className='m-0 p-1'>
                                        <option value="">All</option>
                                        {[...new Set(visitSupplier_Payments_In
                                            .filter(data => data.supplierName === selectedSupplier)
