...
export default function VisitAgentPayOutDetails() {
    ...
    const handleUpdatePerson = async () => {
        setLoading5(true)
        try {
            const response = await fetch(`${apiUrl}/auth/visit/agents/payment_out/update/single/person`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    "Authorization": `Bearer ${user.token}`,
    ...
    const handleUpdate = async () => {
        ...
        try {
            const response = await fetch(`${apiUrl}/auth/visit/agents/update/single/payment_out`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    "Authorization": `Bearer ${user.token}`,
...
const changeStatus = async (myStatus) => {
    if (window.confirm(`Are you sure you want to Change the Status of ${selectedSupplier}?`)) {
      ...
      try {
        const response = await fetch(`${apiUrl}/auth/visit/agents/update/payment_out/status`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
            "Authorization": `Bearer ${user.token}`,
        ...
        <>
            {!option &&
                <>
                    ...
                    {(isLoading && visitAgent_Payments_Out.length<1) &&
                    ...
                    {(!isLoading || visitAgent_Payments_Out.length>0) &&
                        <div className='col-md-12'>
