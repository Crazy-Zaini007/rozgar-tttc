...
import { addFinalStatus } from '../../../redux/reducers/settingSlice';
...
export default function FinalStaus() {
  ...
  const handleForm = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch('https://api-rozgar-tttc.onrender.com/auth/setting/entry/add_final_status', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        dispatch(addFinalStatus(json.data)); // Dispatch the action with received data
        setFinal_Status('')
      }
    } catch (error) {
...
const handleUpdate = async () => {
  ...
  try {
    const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/setting/entry/update_final_status`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        "Authorization": `Bearer ${user.token}`,
      },
      body: JSON.stringify({ myId, final_Status: editedEntry.final_Status})
    })
...
const deleteSupplier = async (data) => {
  if (window.confirm('Are you sure you want to delete this record?')){
    ...
    let myId = data._id
    try {
      const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/setting/entry/delete_final_status`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `Bearer ${user.token}`,
        ...
        <form className='px-2 py-3 rounded' onSubmit={handleForm}>
          <div className="row">
            <div class="col-lg-3 col-md-6 col-sm-12 mb-3">
              ...
              <input type="text" value={final_Status} onChange={(e) => setFinal_Status(e.target.value)} required />
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell className='label text-center'>Final Status</TableCell>
            ...
            <TableBody>
              {finalStatus && finalStatus.length > 0 ? finalStatus.map((data,index)=>(
                <React.Fragment key={index}>
                    ...
                    {editMode && editedRowIndex === index ? (
                      <>
                       <TableCell className='border data_td p-1 '>
                                ...
                                <TableCell className='border data_td p-1 '>
                                  <input type='text' value={editedEntry.final_Status} onChange={(e) => handleInputChange(e, 'final_Status')} required/>
                                </TableCell>
                  ...
                  <TableCell className='data_td text-center'>{data.final_Status}</TableCell>
