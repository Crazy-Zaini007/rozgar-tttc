...
import { addCompany } from '../../../redux/reducers/settingSlice';
...
export default function Companies() {
  ...
  const handleForm = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch('https://api-rozgar-tttc.onrender.com/auth/setting/entry/add_company', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        dispatch(addCompany(json.data)); // Dispatch the action with received data
        setCompany('')
      }
    } catch (error) {
...
const handleUpdate = async () => {
  ...
  try {
    const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/setting/entry/update_company`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        "Authorization": `Bearer ${user.token}`,
      },
      body: JSON.stringify({ myId, company: editedEntry.company})
    })
...
const deleteSupplier = async (data) => {
  if (window.confirm('Are you sure you want to delete this record?')){
    ...
    let myId = data._id
    try {
      const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/setting/entry/delete_company`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `Bearer ${user.token}`,
          ...
          <Table>
            <TableHead >
              <TableRow>
                ...
                <TableCell className='label text-center'>Company Name</TableCell>
            ...
            <TableBody>
              {companies && companies.length > 0 ? companies.map((data,index)=>(
                <React.Fragment key={index}>
                    ...
                    {editMode && editedRowIndex === index ? (
                      <>
                       <TableCell className='border data_td p-1 '>
                                ...
                                <TableCell className='border data_td p-1 '>
                                  <input type='text' value={editedEntry.company} onChange={(e) => handleInputChange(e, 'company')} required/>
                                </TableCell>
                  ...
                  <TableCell className='data_td text-center'>{data.company}</TableCell>
