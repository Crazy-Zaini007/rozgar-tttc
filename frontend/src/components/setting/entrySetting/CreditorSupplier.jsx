...
import { addCrediterSupplierParty } from '../../../redux/reducers/settingSlice';
import CloseIcon from '@mui/icons-material/Close';
import CheckIcon from '@mui/icons-material/Check';
export default function CreditorSupplier() {
  ...
  const addSuppliers = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${apiUrl}/auth/setting/entry/add_creditor_supplier`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        dispatch(addCrediterSupplierParty(json.data)); // Dispatch the action with received data
        setSupplerName('')
        setSupplierCompany('')
        setContact('')
        setCountry('')
...
const handleUpdate = async () => {
  ...
  try {
    const response = await fetch(`${apiUrl}/auth/setting/entry/update_creditor_supplier`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        "Authorization": `Bearer ${user.token}`,
...
const deleteSupplier = async (data) => {
  if (window.confirm('Are you sure you want to delete this record?')){
    ...
    try {
      const response = await fetch(`${apiUrl}/auth/setting/entry/delete_creditor_supplier`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `Bearer ${user.token}`,
      ...
      <div className="col-md-12 details_table mt-4">
        ...
        <TableContainer component={Paper}>
          <Table>
            ...
            <TableBody>
              {crediterSuppliers && crediterSuppliers.length > 0 ? crediterSuppliers.map((data,index)=>(
