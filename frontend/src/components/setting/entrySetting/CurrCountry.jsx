...
import { addCurrCountry } from '../../../redux/reducers/settingSlice';
import CloseIcon from '@mui/icons-material/Close';
import CheckIcon from '@mui/icons-material/Check';
export default function CurrCountry() {
  ...
  const handleForm = async (e) => {
    ...
    try {
      const response = await fetch(`${apiUrl}/auth/setting/entry/add_curr_country`, {
        ...
        body: JSON.stringify({ currCountry }),
      });
      ...
      if (response.ok) {
        ...
        dispatch(addCurrCountry(json.data)); // Dispatch the action with received data
        setCurrCountry('')
      }
    } catch (error) {
...
const handleUpdate = async () => {
  ...
  try {
    const response = await fetch(`${apiUrl}/auth/setting/entry/update_curr_country`, {
      ...
      body: JSON.stringify({ myId, currCountry: editedEntry.currCountry})
    })
  ...
  return (
    <>
      <div className="col-md-12 adding_form">
        <h5>Currency Country</h5>
        <form className='px-2 py-3 rounded' onSubmit={handleForm}>
          <div className="row">
            <div class="col-lg-3 col-md-6 col-sm-12 mb-3">
              <label>Currency Country</label>
              <input type="text" value={currCountry} onChange={(e) => setCurrCountry(e.target.value)} required />
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell className='label text-center'>Currecny Country</TableCell>
            ...
            <TableBody>
              {currCountries && currCountries.length > 0 ? currCountries.map((data,index)=>(
                <React.Fragment key={index}>
                    ...
                    {editMode && editedRowIndex === index ? (
                      <>
                       <TableCell className='border data_td p-1 '>
                                ...
                                <TableCell className='border data_td p-1 '>
                                  <input type='text' value={editedEntry.currCountry} onChange={(e) => handleInputChange(e, 'currCountry')} required/>
                                </TableCell>
                  ...
                  <TableCell className='data_td text-center'>{data.currCountry}</TableCell>
