...
import { addPaymentVia } from '../../../redux/reducers/settingSlice';
...
export default function PaymentVia() {
  ...
  const handleForm = async (e) => {
    ...
    try {
      const response = await fetch(`${apiUrl}/auth/setting/entry/add_payment_via`, {
        ...
        body: JSON.stringify({ payment_Via }),
      });
      ...
      if (response.ok) {
        ...
        dispatch(addPaymentVia(json.data)); // Dispatch the action with received data
        setPayment_Via('')
      }
    } catch (error) {
...
const handleUpdate = async () => {
  ...
  try {
    const response = await fetch(`${apiUrl}/auth/setting/entry/update_payment_via`, {
      ...
      body: JSON.stringify({ myId, payment_Via: editedEntry.payment_Via})
    })
        ...
        <form className='px-2 py-3 rounded' onSubmit={handleForm}>
          <div className="row">
            <div class="col-lg-3 col-md-6 col-sm-12 mb-3">
              ...
              <input type="text" value={payment_Via} onChange={(e) => setPayment_Via(e.target.value)} required />
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell className='label text-center'>Payment Via</TableCell>
                <TableCell className='label text-center'>Action</TableCell>
              </TableRow>
            </TableHead>

            <TableBody>
              {paymentVia && paymentVia.length > 0 ? paymentVia.map((data,index)=>(
                <React.Fragment key={index}>
                    ...
                    {editMode && editedRowIndex === index ? (
                      <>
                       <TableCell className='border data_td p-1 '>
                                ...
                                <TableCell className='border data_td p-1 '>
                                  <input type='text' value={editedEntry.payment_Via} onChange={(e) => handleInputChange(e, 'payment_Via')} required/>
                                </TableCell>
                  ...
                  <TableCell className='data_td text-center'>{data.payment_Via}</TableCell>
