...
import { addVisitSalesParty } from '../../../redux/reducers/settingSlice';
import CloseIcon from '@mui/icons-material/Close';
import CheckIcon from '@mui/icons-material/Check';
export default function VISP() {
  ...
  const addSuppliers = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${apiUrl}/auth/setting/entry/add_visp`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        dispatch(addVisitSalesParty(json.data)); // Dispatch the action with received data
        setSupplerName('')
        setSupplierCompany('')
        setContact('')
        setCountry('')
...
const handleUpdate = async () => {
  ...
  try {
    const response = await fetch(`${apiUrl}/auth/setting/entry/update_visp`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        "Authorization": `Bearer ${user.token}`,
...
const deleteSupplier = async (data) => {
  if (window.confirm('Are you sure you want to delete this record?')){
    ...
    try {
      const response = await fetch(`${apiUrl}/auth/setting/entry/delete_visp`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `Bearer ${user.token}`,
      ...
      <div className="col-md-12 details_table mt-4">
        ...
        <TableContainer component={Paper}>
          <Table>
            ...
            <TableBody>
              {visitSalesParties && visitSalesParties.length > 0 ? visitSalesParties.map((data,index)=>(
