...
import { addEntryMode } from '../../../redux/reducers/settingSlice';
  ...
  const handleForm = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        dispatch(addEntryMode(json.data)); // Dispatch the action with received data
        setEntry_Mode('')
      }
    } catch (error) {
...
const handleUpdate = async () => {
  ...
  try {
    const response = await fetch(`/auth/setting/entry/update_entry_mode`, {
      ...
      body: JSON.stringify({ myId, entry_Mode: editedEntry.entry_Mode})
    })
        ...
        <form className='px-2 py-3 rounded' onSubmit={handleForm}>
          <div className="row">
            <div class="col-lg-3 col-md-6 col-sm-12 mb-3">
              ...
              <input type="text" value={entry_Mode} onChange={(e) => setEntry_Mode(e.target.value)} required />
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell className='label text-center'>Entry Mode</TableCell>
            ...
            <TableBody>
              {entryMode && entryMode.length > 0 ? entryMode.map((data,index)=>(
                    ...
                    {editMode && editedRowIndex === index ? (
                      <>
                       <TableCell className='border data_td p-1 '>
                                ...
                                <TableCell className='border data_td p-1 '>
                                  <input type='text' value={editedEntry.entry_Mode} onChange={(e) => handleInputChange(e, 'entry_Mode')} required/>
                                </TableCell>
