...
import { addTicketPurchaseParty } from '../../../redux/reducers/settingSlice';
...
export default function TPP(){
  ...
  const addSuppliers = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch('https://api-rozgar-tttc.onrender.com/auth/setting/entry/add_tpp', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        dispatch(addTicketPurchaseParty(json.data)); // Dispatch the action with received data
        setSupplerName('')
        setSupplierCompany('')
        setContact('')
        setCountry('')
...
const handleUpdate = async () => {
  ...
  try {
    const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/setting/entry/update_tpp`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        "Authorization": `Bearer ${user.token}`,
...
const deleteSupplier = async (data) => {
  if (window.confirm('Are you sure you want to delete this record?')){
    ...
    try {
      const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/setting/entry/delete_tpp`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `Bearer ${user.token}`,
      ...
      <div className="col-md-12 details_table mt-4">
        ...
        <TableContainer component={Paper}>
          <Table>
            ...
            <TableBody>
              {ticketPurchaseParties && ticketPurchaseParties.length > 0 ? ticketPurchaseParties.map((data,index)=>(
