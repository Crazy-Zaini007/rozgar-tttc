...
import { addVisitPurchaseParty } from '../../../redux/reducers/settingSlice';
import CloseIcon from '@mui/icons-material/Close';
import CheckIcon from '@mui/icons-material/Check';
export default function VIPP() {
  ...
  const addSuppliers = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch('https://api-rozgar-tttc.onrender.com/auth/setting/entry/add_vipp', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        dispatch(addVisitPurchaseParty(json.data)); // Dispatch the action with received data
        setSupplerName('')
        setSupplierCompany('')
        setContact('')
        setCountry('')
...
const handleUpdate = async () => {
  ...
  try {
    const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/setting/entry/update_vipp`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        "Authorization": `Bearer ${user.token}`,
...
const deleteSupplier = async (data) => {
  if (window.confirm('Are you sure you want to delete this record?')){
    ...
    try {
      const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/setting/entry/delete_vipp`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `Bearer ${user.token}`,
      ...
      <div className="col-md-12 details_table mt-4">
        ...
        <TableContainer component={Paper}>
          <Table>
            ...
            <TableBody>
              {visitPurchaseParties && visitPurchaseParties.length > 0 ? visitPurchaseParties.map((data,index)=>(
