...
import { addAssets } from '../../../redux/reducers/settingSlice';
...
export default function Assets() {
  ...
  const addSuppliers = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${apiUrl}/auth/setting/entry/add_asset`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${user.token}`,
        },
        body: JSON.stringify({ assetName, picture }),
      });
      ...
      if (response.ok) {
        ...
        dispatch(addAssets(json.data)); // Dispatch the action with received data
        setAssetName('')
        setSupplierCompany('')
        setContact('')
        setCountry('')
...
const handleUpdate = async () => {
  ...
  let assetId = editedEntry._id
  try {
    const response = await fetch(`${apiUrl}/auth/setting/entry/update_asset`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        "Authorization": `Bearer ${user.token}`,
      },
      body: JSON.stringify({ assetId, assetName: editedEntry.assetName, picture: editedEntry.picture })
    })
...
const deleteSupplier = async (data) => {
  if (window.confirm('Are you sure you want to delete this record?')){
    ...
    let assetId = data._id
    try {
      const response = await fetch(`${apiUrl}/auth/setting/entry/delete_asset`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `Bearer ${user.token}`,
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell className='label text-center'>Asset Name</TableCell>
            ...
            <TableBody>
              {assets && assets.length > 0 ? assets.map((data,index)=>(
                <React.Fragment key={index}>
                    ...
                    {editMode && editedRowIndex === index ? (
                      <>
                       <TableCell className='border data_td p-1 '>
                                ...
                                <TableCell className='border data_td p-1 '>
                                  <input type='text' value={editedEntry.assetName} onChange={(e) => handleInputChange(e, 'assetName')} required/>
                                </TableCell>
                                {/* <TableCell className='border data_td p-1 '>
                  ...
                  <TableCell className='data_td text-center'>{data.assetName}</TableCell>
