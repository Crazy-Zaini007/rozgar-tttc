...
import { addCategory } from '../../../redux/reducers/settingSlice';
...
export default function Categories() {
  ...
  const handleForm = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${apiUrl}/auth/setting/entry/add_category`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        dispatch(addCategory(json.data)); // Dispatch the action with received data
        setCategory('')
      }
    } catch (error) {
...
const handleUpdate = async () => {
  ...
  try {
    const response = await fetch(`${apiUrl}/auth/setting/entry/update_category`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        "Authorization": `Bearer ${user.token}`,
...
const deleteSupplier = async (data) => {
  if (window.confirm('Are you sure you want to delete this record?')){
    ...
    let myId = data._id
    try {
      const response = await fetch(`${apiUrl}/auth/setting/entry/delete_category`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `Bearer ${user.token}`,
