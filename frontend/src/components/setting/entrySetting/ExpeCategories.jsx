...
import { addExpeCategory } from '../../../redux/reducers/settingSlice';
...
export default function ExpeCategories() {
  ...
  const handleForm = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${apiUrl}/auth/setting/entry/add_expense_category`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        dispatch(addExpeCategory(json.data)); // Dispatch the action with received data
        setCategory('')
      }
    } catch (error) {
...
const handleUpdate = async () => {
  ...
  try {
    const response = await fetch(`${apiUrl}/auth/setting/entry/update_expense_category`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        "Authorization": `Bearer ${user.token}`,
      },
      body: JSON.stringify({ myId, category: editedEntry.category})
    })
...
const deleteSupplier = async (data) => {
  if (window.confirm('Are you sure you want to delete this record?')){
    ...
    let myId = data._id
    try {
      const response = await fetch(`${apiUrl}/auth/setting/entry/delete_expense_category`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `Bearer ${user.token}`,
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell className='label text-center'>Category Name</TableCell>
            ...
            <TableBody>
              {expenseCategories && expenseCategories.length > 0 ? expenseCategories.map((data,index)=>(
                <React.Fragment key={index}>
                    ...
                    {editMode && editedRowIndex === index ? (
                      <>
                       <TableCell className='border data_td p-1 '>
                                ...
                                <TableCell className='border data_td p-1 '>
                                  <input type='text' value={editedEntry.category} onChange={(e) => handleInputChange(e, 'category')} required/>
                                </TableCell>
                  ...
                  <TableCell className='data_td text-center'>{data.category}</TableCell>
