...
export default function TicketCandPaymentOutDetails() {
  ...
  const {  getTicketCandPaymentsOut } = TicketHook()
  ...
  const fetchData = async () => {
    ...
    try {
      setIsLoading(true)
      await  getTicketCandPaymentsOut();
      setIsLoading(false);
  ...
  const deletePaymentIn = async (payment) => {
    if (window.confirm('Are you sure you want to delete this record?')){
      ...
      try {
        const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/ticket/candidates/delete/single/payment_out`, {
          method: 'DELETE',
          headers: {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/ticket/candidates/update/single/payment_out`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `Bearer ${user.token}`,
  ...
  const handleTotalPaymentUpdate = async () => {
    setLoading3(true)
    try {
      const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/ticket/candidates/update/all/payment_out`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `Bearer ${user.token}`,
  ...
  const deleteTotalpayment = async (person) => {
    if (window.confirm('Are you sure you want to delete this record?')){
      setLoading5(true)
      try {
        const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/ticket/candidates/delete/all/payment_out`, {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
            "Authorization": `Bearer ${user.token}`,
  ...
  const filteredTotalPaymentOut = ticketCand_Payments_Out.filter(payment => {
    return (
  ...
  const filteredIndividualPayments = ticketCand_Payments_Out
          ...
          <div className='col-md-12 '>
            <Paper className='py-3 mb-2 px-2 d-flex justify-content-between'>
              ...
              <div className="right d-flex">
                {ticketCand_Payments_Out.length > 0 &&
                  <>
              ...
              <div className="row">
                <div className="col-auto px-1">
                  ...
                  <select value={date1} onChange={(e) => setDate1(e.target.value)} className='m-0 p-1'>
                    ...
                    {[...new Set(ticketCand_Payments_Out.map(data => data.createdAt))].map(dateValue => (
                ...
                <div className="col-auto px-1">
                  ...
                  <select value={name} onChange={(e) => setName(e.target.value)} className='m-0 p-1'>
                    <option value="">All</option>
                    {ticketCand_Payments_Out && ticketCand_Payments_Out.map((data) => (
              ...
              <div className="row">
                <div className="col-auto px-1">
                  ...
                  <select value={date2} onChange={(e) => setDate2(e.target.value)} className='m-0 p-1'>
                    <option value="">All</option>
                    {[...new Set(ticketCand_Payments_Out
                      .filter(data => data.supplierName === selectedSupplier)
