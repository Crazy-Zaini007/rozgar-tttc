...
export default function TicketAgentPayOutDetails() {
...
const changeStatus = async (myStatus) => {
    if (window.confirm(`Are you sure you want to Change the Status of ${selectedSupplier}?`)) {
      ...
      try {
        const response = await fetch(`${apiUrl}/auth/ticket/agents/update/payment_out/status`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
            "Authorization": `Bearer ${user.token}`,
        ...
        <>
            {!option &&
                <>
                    ...
                    {(isLoading && ticketAgent_Payments_Out.length<1) &&
                    ...
                    {(!isLoading || ticketAgent_Payments_Out.length>0) &&
                        <div className='col-md-12'>
                                ...
                                <TableContainer sx={{ maxHeight: 600 }}>
                                    <Table stickyHeader>
                                        <TableHead>
                                            ...
                                            <TableCell className='label border' >Total Ticket Visa Price PKR</TableCell>
