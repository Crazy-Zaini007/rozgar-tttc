...
export default function AzadVisaSupPayOutDetails() {
    ...
    const deletePaymentIn = async (payment) => {
        if (window.confirm('Are you sure you want to delete this record?')){
            ...
            try {
                const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/ticket/suppliers/delete/single/payment_out`, {
                    method: 'DELETE',
                    headers: {
    ...
    const deletePerson = async (person) => {
        if (window.confirm('Are you sure you want to delete this record?')){
            ...
            try {
                const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/ticket/suppliers/delete/person/payment_out`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        "Authorization": `Bearer ${user.token}`,
    ...
    const handleUpdatePerson = async () => {
        setLoading5(true)
        try {
            const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/ticket/suppliers/payment_out/update/single/person`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    "Authorization": `Bearer ${user.token}`,
    ...
    const handleUpdate = async () => {
        ...
        try {
            const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/ticket/suppliers/update/single/payment_out`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    "Authorization": `Bearer ${user.token}`,
    ...
    const deleteTotalpayment = async (person) => {
        if (window.confirm('Are you sure you want to delete this record?')){
            setLoading5(true)
            try {
                const response = await fetch(`https://api-rozgar-tttc.onrender.com/auth/ticket/suppliers/delete/all/payment_out`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        "Authorization": `Bearer ${user.token}`,
    ...
    const filteredTotalPaymentIn = ticketSupplier_Payments_Out.filter(payment => {
        return (
    ...
    const filteredIndividualPayments = ticketSupplier_Payments_Out
  ...
  `;
    ...
    const filteredPersons = ticketSupplier_Payments_Out
        .filter((data) => data.supplierName === selectedSupplier)
        .map((filteredData) => ({
            ...filteredData,
                            ...
                            <div className="row">
                                <div className="col-auto px-1">
                                    ...
                                    <select value={date1} onChange={(e) => setDate1(e.target.value)} className='m-0 p-1'>
                                        ...
                                        {[...new Set(ticketSupplier_Payments_Out.map(data => data.createdAt))].map(dateValue => (
                                ...
                                <div className="col-auto px-1">
                                    ...
                                    <select value={supplier1} onChange={(e) => setSupplier1(e.target.value)} className='m-0 p-1'>
                                        <option value="">All</option>
                                        {ticketSupplier_Payments_Out && ticketSupplier_Payments_Out.map((data) => (
                        ...
                        <div className='col-md-12'>
                            <Paper className='py-3 mb-1 px-2 detail_table'>
                                ...
                                <TablePagination
                                    ...
                                    component='div'
                                    count={ticketSupplier_Payments_Out.length}
                            ...
                            <div className="row">
                                <div className="col-auto px-1">
                                    ...
                                    <select value={date2} onChange={(e) => setDate2(e.target.value)} className='m-0 p-1'>
                                        <option value="">All</option>
                                        {[...new Set(ticketSupplier_Payments_Out
                                            .filter(data => data.supplierName === selectedSupplier)
