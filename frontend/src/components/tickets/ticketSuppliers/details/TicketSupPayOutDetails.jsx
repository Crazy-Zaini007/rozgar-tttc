...
const changeStatus = async (myStatus) => {
    if (window.confirm(`Are you sure you want to Change the Status of ${selectedSupplier}?`)) {
      ...
      try {
        const response = await fetch(`${apiUrl}/auth/ticket/suppliers/update/payment_out/status`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
            "Authorization": `Bearer ${user.token}`,
        ...
        <>
            {!option &&
                <>
                    ...
                    {(isLoading && ticketSupplier_Payments_Out.length<1) &&
                    ...
                    {(!isLoading || ticketSupplier_Payments_Out.length>0) &&
                        <div className='col-md-12'>
