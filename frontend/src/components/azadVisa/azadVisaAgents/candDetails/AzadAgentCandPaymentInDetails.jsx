...
export default function AzadAgentCandPaymentInDetails() {
  ...
  const deletePaymentIn = async (payment) => {
    if (window.confirm('Are you sure you want to delete this record?')) {
      ...
      try {
        const response = await fetch(`${apiUrl}/auth/azadVisa/agents/delete/cand_vise/payment_in`, {
          method: 'DELETE',
          headers: {
  ...
  const deleteSinglePaymentIn = async (payment) => {
    if (window.confirm('Are you sure you want to delete this record?')) {
      ...
      try {
        const response = await fetch(`${apiUrl}/auth/azadVisa/agents/delete/cand_vise/single/payment_in`, {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
            "Authorization": `Bearer ${user.token}`,
  ...
  const handleUpdate = async () => {
    ...
    let paymentId = details._id
    try {
      const response = await fetch(`${apiUrl}/auth/azadVisa/agents/update/cand_vise/single/payment_in`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `Bearer ${user.token}`,
  ...
  const handleTotalPaymentUpdate = async () => {
    setLoading3(true)
    try {
      const response = await fetch(`${apiUrl}/auth/azadVisa/agents/update/all/payment_in`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `Bearer ${user.token}`,
        },
        body: JSON.stringify({ supplierName: editedEntry1.supplierName, total_Payment_In: editedEntry1.total_Payment_In, total_Cash_Out: editedEntry1.total_Cash_Out, total_Azad_Visa_Price_In_Curr: editedEntry1.total_Payment_In_Curr, open: editedEntry1.open, close: editedEntry1.close })
      })
  ...
  const downloadExcel = () => {
    ...
    filteredTotalPaymentIn.forEach((payments, index) => {
      const rowData = {
        ...
        Total_Azad_Visa_Price_In_PKR: payments.total_Azad_Visa_Price_In_PKR,
        ...
        Total_Azad_Visa_Price_In_Curr: payments.total_Azad_Visa_Price_In_Curr,
 ...
 const handleUpdatePayment = async () => {
  ...
  try {
    const response = await fetch(`${apiUrl}/auth/azadVisa/agents/update/cand_vise/payment_in`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        "Authorization": `Bearer ${user.token}`,
          ...
          <div className='col-md-12 '>
            <Paper className='py-3 mb-2 px-2 d-flex justify-content-between'>
              <div className="left d-flex">
                <h4>Azad Agent Cand-Vise PaymentIn Details</h4>
              </div>
                            ...
                            <TableRow key={entry?._id} className={outerIndex % 2 === 0 ? 'bg_white' : 'bg_dark'} >
                              {editMode1 && editedRowIndex1 === outerIndex ? (
                                ...
                                <>
                                  ...
                                  <TableCell className='border data_td p-1 '>
                                    <input type='number' min='0' value={editedEntry1.total_Azad_Visa_Price_In_PKR} onChange={(e) => handleTotalPaymentInputChange(e, 'total_Azad_Visa_Price_In_PKR')} readonly />
                                  </TableCell>
                                  <TableCell className='border data_td p-1 '>
                                  ...
                                  <TableCell className='border data_td p-1 '>
                                    <input type='number' value={editedEntry1.total_Azad_Visa_Price_In_PKR - editedEntry1.total_Payment_In + editedEntry1.total_Cash_Out} readonly />
                                  </TableCell>
                                  <TableCell className='border data_td p-1 '>
                                    <input type='number' min='0' value={editedEntry1.total_Azad_Visa_Price_In_Curr} onChange={(e) => handleTotalPaymentInputChange(e, 'total_Azad_Visa_Price_In_Curr')} readonly />
                                  </TableCell>
                                  <TableCell className='border data_td p-1 '>
                                  ...
                                  <TableCell className='border data_td p-1 '>
                                    <input type='number' min='0' value={editedEntry1.total_Azad_Visa_Price_In_Curr - editedEntry1.total_Payment_In_Curr} onChange={(e) => handleTotalPaymentInputChange(e, 'remaining_Curr')} readonly />
                                  </TableCell>
