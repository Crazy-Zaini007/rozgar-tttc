    ...
    if (window.confirm('Are you sure you want to delete this record?')){
        ...
        try {
            const response = await fetch(`${apiUrl}/auth/azadVisa/agents/delete/single/payment_in`, {
                ...
                body: JSON.stringify({ paymentId,newStatus, supplierName: selectedSupplier, payment_In: payment.payment_In, payment_Via: payment.payment_Via, cash_Out: payment.cash_Out, curr_Amount: payment.curr_Amount })
            })
    ...
    if (window.confirm('Are you sure you want to delete this record?')){
        ...
        try {
            const response = await fetch(`${apiUrl}/auth/azadVisa/agents/delete/person/payment_in`, {
                ...
                body: JSON.stringify({ personId,newStatus, supplierName: selectedSupplier, azad_Visa_Price_In_PKR: person.azad_Visa_Price_In_PKR, azad_Visa_Price_In_Curr: person.azad_Visa_Price_In_Curr })
            })
        ...
        <>
            {!option &&
                <>
                    ...
                    {(isLoading && azadAgent_Payments_In.length<1) &&
                    ...
                    {(!isLoading || azadAgent_Payments_In.length>0)&&
                        <div className='col-md-12'>
                ...
                <>
                    ...
                    <div className="col-md-12 my-2">
                        <div className="d-flex justify-content-between supplier_Name">
                            ...
                            <div className="right">
                            {azadAgent_Payments_In && azadAgent_Payments_In.some((data)=>data.supplierName===selectedSupplier && data.status.toLowerCase()==='open')&&
                            ...
                            <Table stickyHeader>
                                ...
                                <TableBody>
                                    {filteredPersons.map((filteredData) => (
                                        <>
                                            {filteredData.personsslice(0,rowsValue1 ? rowsValue1 : undefined).map((person, index) => (
  ...
  <div className="modal-dialog modal-fullscreen">
    <div className="modal-content">
      <div className="modal-header">
       ...
       <span className='mx-1'>Total: {azadAgent_Payments_In.filter((data)=>data.supplierName===selectedSupplier&&data._id===newStatus).map(data=>data.total_Azad_Visa_Price_In_PKR||0)} |</span>
       <span className='mx-1'>Total Payment done: {azadAgent_Payments_In.filter((data)=>data.supplierName===selectedSupplier&&data._id===newStatus).map(data=>data.total_Payment_In||0)} |</span>
       <span className='mx-1'>Remaining Balance: {azadAgent_Payments_In.filter((data)=>data.supplierName===selectedSupplier&&data._id===newStatus).map(data=>data.remaining_Balance||0)} </span>
                          ...
                          {editMode2 && editedRowIndex2 === index ? (
                            ...
                            {showFlightDate &&
                             ...
                             <input type='date' value={editedEntry2.flight_Date} onChange={(e) => handlePersonInputChange(e, 'flight_Date')} />
                              ...
                              <TableCell className='border data_td p-1 '>
                                <input type='number' value={editedEntry2.azad_Visa_Price_In_PKR - editedEntry2.total_In + editedEntry2.cash_Out} readonly />
                              </TableCell>
                              {show && <TableCell className='border data_td p-1 '>
                                <input type='number' value={editedEntry2.azad_Visa_Price_In_Curr} readonly />
                              </TableCell>}
                            ...
                            <TableCell className='border data_td p-0 text-center' style={{ width: 'auto' }}>
                              ...
                              <TableCell className='border data_td text-center' >{person?.azad_Visa_Price_In_PKR - person?.total_In + person?.cash_Out}</TableCell>
                              {show && <TableCell className='border data_td text-center' >{person?.azad_Visa_Price_In_Curr}</TableCell>}
                              <TableCell className='border data_td text-center' >{person?.status}</TableCell>
                            </>
                          )}
  ...
  <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
    ...
    <li><a class="dropdown-item" onClick={()=>setShowDetails(!showDetails)}>{showDetails?'Hide':'Show'} Details</a></li>
      ...
      <div className="modal-footer">
       ...
       <span className='mx-1'>Opening Balance: {azadAgent_Payments_In.filter((data)=>data.supplierName===selectedSupplier&&data._id===newStatus).map(data=>data.opening||0)} |</span>
       <span className='mx-1'>Closing Balance: {azadAgent_Payments_In.filter((data)=>data.supplierName===selectedSupplier&&data._id===newStatus).map(data=>data.closing||0)}</span>
