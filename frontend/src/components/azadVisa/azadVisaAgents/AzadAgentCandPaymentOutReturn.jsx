...
export default function AzadAgentCandPaymentOutReturn() {
  ...
  const handleForm = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${apiUrl}/auth/agents/payment_out/cash_out`, {
        method: 'POST',
        headers: {
                ...
                <select required value={supplierName}
                  onChange={(e) => {
                    ...
                    if (selectedSupplierValue === "") {
                      ...
                      // Filter agent_Payments_Out based on the selected supplier
                      const selectedSupplierData = agent_Payments_Out.find(
