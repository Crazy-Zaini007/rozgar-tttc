...
export default function AzadVisaSupPayOutDetails() {
    ...
    if (window.confirm('Are you sure you want to delete this record?')){
        ...
        try {
            const response = await fetch(`${apiUrl}/auth/azadVisa/suppliers/delete/single/payment_out`, {
                method: 'DELETE',
                headers: {
    ...
    if (window.confirm('Are you sure you want to delete this record?')){
        ...
        try {
            const response = await fetch(`${apiUrl}/auth/azadVisa/suppliers/delete/person/payment_out`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    "Authorization": `Bearer ${user.token}`,
    ...
    const handleUpdatePerson = async () => {
        setLoading5(true)
        try {
            const response = await fetch(`${apiUrl}/auth/azadVisa/suppliers/payment_out/update/single/person`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    "Authorization": `Bearer ${user.token}`,
    ...
    const handleUpdate = async () => {
        ...
        try {
            const response = await fetch(`${apiUrl}/auth/azadVisa/suppliers/update/single/payment_out`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    "Authorization": `Bearer ${user.token}`,
    ...
    if (window.confirm('Are you sure you want to delete this record?')){
        setLoading5(true)
        try {
            const response = await fetch(`${apiUrl}/auth/azadVisa/suppliers/delete/all/payment_out`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    "Authorization": `Bearer ${user.token}`,
        ...
        <>
            {!option &&
                <>
                    ...
                    {(isLoading && azadSupplier_Payments_Out.length<1) &&
                  ...
                  <select value={status} onChange={(e) => setStatus(e.target.value)} className='m-0 p-1'>
                    ...
                    {(!isLoading || azadSupplier_Payments_Out.length>0) &&
                        <div className='col-md-12'>
                            ...
                            <Table stickyHeader>
                                <TableHead className="thead">
                                    <TableRow>
                                        ...
                                        {show && <TableCell className='label border'>Azad Visa Price In Oth Curr</TableCell>}
                                        <TableCell className='label border'>Status</TableCell>
