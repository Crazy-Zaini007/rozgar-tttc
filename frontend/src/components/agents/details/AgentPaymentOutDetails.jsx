...
export default function AgentPaymentOutDetails() {
    ...
    if (window.confirm('Are you sure you want to delete this record?')) {
      ...
      try {
        const response = await fetch(`${apiUrl}/auth/agents/delete/single/payment_out`, {
          ...
          body: JSON.stringify({ paymentId,newStatus, supplierName: selectedSupplier, payment_Via: payment.payment_Via, payment_Out: payment.payment_Out, cash_Out: payment.cash_Out, curr_Amount: payment.curr_Amount, cand_Name: payment.cand_Name })
        })
  ...
  const handleTotalPaymentUpdate = async () => {
    ...
    try {
      const response = await fetch(`${apiUrl}/auth/agents/update/all/payment_out`, {
        ...
        body: JSON.stringify({ newStatus,supplierName: editedEntry1.supplierName, total_Payment_Out: editedEntry1.total_Payment_Out, total_Cash_Out: editedEntry1.total_Cash_Out, total_Visa_Price_Out_Curr: editedEntry1.total_Visa_Price_Out_Curr, open: editedEntry1.open, close: editedEntry1.close })
      })
  ...
  const filteredTotalPaymentOut = agent_Payments_Out.filter(payment => {
    return (
  ...
  const filteredIndividualPayments = agent_Payments_Out
  ...
  const filteredPersons = agent_Payments_Out
    .filter((data) => data.supplierName === selectedSupplier&&data.status===newStatus)
    .map((filteredData) => ({
      ...filteredData,
    ...
    const filteredClosingPersons = agent_Payments_Out
    .filter((data) => data.supplierName === selectedSupplier&&data.status===newStatus)
    .map((filteredData) => ({
      ...filteredData,
      persons: filteredData.persons
  ...
  const changeStatus = async (myStatus) => {
    if (window.confirm(`Are you sure you want to Change the Status of ${selectedSupplier}?`)) {
      ...
      try {
        const response = await fetch(`${apiUrl}/auth/agents/update/payment_out/status`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
            "Authorization": `Bearer ${user.token}`,
  ...
  const printMainTable = () => {
    ...
    const printContentString = `
      ...
      <div class="print-header">
        <h1 class="title">Agents Payment Out Details</h1>
      </div>
      <hr/>
      <table class='print-table'>
        ...
        <tr>
          ...
          <td>${String(filteredTotalPaymentOut.reduce((total, entry) => total + (entry.remaining_Balance), 0))}</td>
          <td>${String(filteredTotalPaymentOut.reduce((total, entry) => total + entry.total_Visa_Price_Out_Curr, 0))}</td>
          <td>${String(filteredTotalPaymentOut.reduce((total, entry) => total + entry.total_Payment_Out_Curr, 0))}</td>
          <td>${String(filteredTotalPaymentOut.reduce((total, entry) => total + (entry.total_Visa_Price_Out_Curr - entry.total_Payment_Out_Curr), 0))}</td>
          <td></td>
        </tr>
      </tbody>
    ...
    if (printWindow) {
      ...
      printWindow.document.write(`
        <html>
          <head>
            <title>Agents Payment Out Details</title>
          </head>
    ...
    <>
      {!option &&
        <>
          ...
          {(isLoading && agent_Payments_Out.length<1) &&
                            ...
                            <TableRow key={entry?._id} className={outerIndex % 2 === 0 ? 'bg_white' : 'bg_dark'} >
                              {editMode1 && editedRowIndex1 === outerIndex ? (
                                ...
                                <>
                                  ...
                                  <TableCell className='border data_td p-1 '>
                                    <input type='text' value={editedEntry1.close} onChange={(e) => handleTotalPaymentInputChange(e, 'close')} />
                                  </TableCell>
                                  <TableCell className='border data_td p-1 '>
                                    <input type='text' value={editedEntry1.open} onChange={(e) => handleTotalPaymentInputChange(e, 'open')} />
                                  </TableCell>
        ...
        <>
          ...
          <div className="col-md-12 my-2 p-0">
            <div className="d-flex justify-content-between supplier_Name">
              ...
              <div className="right">
              {agent_Payments_Out && agent_Payments_Out.some((data)=>data.supplierName===selectedSupplier && data.status.toLowerCase()==='open')&&
  ...
  <div className="modal-dialog modal-fullscreen">
    <div className="modal-content">
      <div className="modal-header">
       ...
       <span className='mx-1'>Total: {agent_Payments_Out.filter((data)=>data.supplierName===selectedSupplier&&data.status===newStatus).map(data=>data.total_Visa_Price_Out_PKR||0)} |</span>
       <span className='mx-1'>Total Payment done: {agent_Payments_Out.filter((data)=>data.supplierName===selectedSupplier&&data.status===newStatus).map(data=>data.total_Payment_Out||0)} |</span>
       <span className='mx-1'>Remaining Balance: {agent_Payments_Out.filter((data)=>data.supplierName===selectedSupplier&&data.status===newStatus).map(data=>data.remaining_Balance||0)} </span>
  ...
  <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
    ...
    <li><a class="dropdown-item" onClick={()=>setShowDetails(!showDetails)}>{showDetails?'Hide':'Show'} Details</a></li>
      ...
      <div className="modal-footer">
       ...
       <span className='mx-1'>Opening Balance: {agent_Payments_Out.filter((data)=>data.supplierName===selectedSupplier&&data.status===newStatus).map(data=>data.opening||0)} |</span>
       <span className='mx-1'>Closing Balance: {agent_Payments_Out.filter((data)=>data.supplierName===selectedSupplier&&data.status===newStatus).map(data=>data.closing||0)}</span>
