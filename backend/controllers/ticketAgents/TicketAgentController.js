  ...
  try {
    ...
    try {
      ...
      for (const payment of multiplePayment) {
        ...
        if(!existingSupplier){
          payment.nameError='Agent not found in Payments records'
          confirmStatus=false
         }
...
const updateAgentPaymentInPerson = async (req, res) => {
  ...
  if (user) {
    try {
      ...
      const agents = await TicketAgents.find({});
      ...
      const ticketAgents = await AzadAgents.find({});
...
const getAllAzadAgentPaymentsIn = async (req, res) => {
  try {
    ...
    if (user.role === "Admin") {
      const supplierPayments = await TicketAgents.find({}).sort({
        createdAt: -1,
      });
...
const changeAgentPaymentInStatus = async (req, res) => {
  try {
    ...
    const newSupplier=new TicketAgents({
      payment_In_Schema:{
    ...
    let responseMessage;
        ...
        const newNotification=new Notifications({
          ...
          content:`${user.userName} Closed Khata with Ticket Agent:${supplierName} and new Khata created successfully`,
          date: new Date().toISOString().split("T")[0]
...
const addAgentCandVisePaymentIn = async (req, res) => {
  try {
    ...
    const newNotification = new Notifications({
      type: "Ticket Agent Cand-Wise Payment In",
...
const deleteAgentCandVisePaymentIn = async (req, res) => {
  try {
    ...
    if (paymentToDelete) {
      ...
      await existingSupplier.save();
      const newNotification = new Notifications({
        type: "Ticket Agents Cand-Wise Payment In Deleted",
...
const updateAgentCandVisePaymentIn = async (req, res) => {
  ...
  if (!existingSupplier) {
    res.status(404).json({ message: "Ticket Agent not found" });
    return;
  }
  ...
  try {
    ...
    if (paymentToFind) {
      ...
      if (candPayment) {
        ...
        await CashInHand.updateOne({}, cashInHandUpdate);
        const newNotification = new Notifications({
          type: "Ticket Agent Cand-Wise Payment In Deleted",
  ...
  try {
    ...
    if (paymentToFind) {
      ...
      if (candPayment) {
        ...
        await CashInHand.updateOne({}, cashInHandUpdate);
        const newNotification = new Notifications({
          type: "Ticket Agent Cand-Wise Payment In Updated",
...
const addAgentCandVisePaymentOut = async (req, res) => {
  try {
    ...
    const newNotification = new Notifications({
      type: "Ticket Agent Cand-Wise Payment Out",
...
const deleteAgentCandVisePaymentOut = async (req, res) => {
  try {
    ...
    if (paymentToDelete) {
      ...
      await existingSupplier.save();
      const newNotification = new Notifications({
        type: "Ticket Agent Cand-Wise Payment Out Deleted",
  ...
  try {
    ...
    if (paymentToFind) {
      ...
      if (candPayment) {
        ...
        await CashInHand.updateOne({}, cashInHandUpdate);
        const newNotification = new Notifications({
          type: "Ticket Agent Cand-Wise Payment Out Updated",
