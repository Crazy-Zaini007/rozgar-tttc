        ...
        try {
          ...
          res
            ...
            .json({
              ...
              message: `Payment In: ${payment_In} added Successfully to ${updatedSupplier.Candidate_Payment_In_Schema.supplierName}'s Record`,
            });
        } catch (error) {
      ...
      for (const payment of multiplePayment) {
        ...
        for (const supplier of suppliers) {
          if (supplier.Candidate_Payment_In_Schema) {
            if (
              supplier.Candidate_Payment_In_Schema.supplierName.toLowerCase() ===
      ...
      if (user.role === "Admin") {
        ...
        try {
          ...
          res
            ...
            .json({
              data: updatedSupplier,
              message: `Cash Out: ${cash_Out} added Successfully to ${updatedSupplier.Candidate_Payment_In_Schema.supplierName}'s Record`,
            });
        } catch (error) {
          console.error("Error updating values:", error);
          res
    ...
    try {
      ...
      await existingSupplier.updateOne({
        $inc: {
          ...
          "Candidate_Payment_In_Schema.total_Payment_In_Curr": curr_Amount
            ? -curr_Amount
            : 0,
          "Candidate_Payment_In_Schema.remaining_Curr": curr_Amount
            ? curr_Amount
            : 0,
        },
  ...
  if (user && user.role === "Admin") {
    try {
      ...
      const paymentToUpdate =
        existingSupplier.Candidate_Payment_In_Schema.payment.find(
      ...
      await existingSupplier.updateOne({
        $inc: {
          ...
          "Candidate_Payment_In_Schema.total_Payment_In_Curr": updateCurr_Amount
            ? -updateCurr_Amount
            : 0,
          "Candidate_Payment_In_Schema.remaining_Curr": updateCurr_Amount
            ? -updateCurr_Amount
            : 0,
        },
      });
...
const deleteAzadCandPaymentInSchema = async (req, res) => {
  ...
  try {
    ...
    cashInHandUpdate.$inc.total_Cash =
      -existingSupplier.Candidate_Payment_In_Schema.total_Payment_In;
    cashInHandUpdate.$inc.total_Cash =
      existingSupplier.Candidate_Payment_In_Schema.total_Cash_Out;
  ...
  if (user && user.role === "Admin") {
    try {
      ...
      if (azadCandidateIn) {
        ...
        azadCandidateIn.Candidate_Payment_In_Schema.flight_Date = flight_Date
          ? flight_Date
          : "Not Fly";
      ...
      if (azadCandidateOut) {
        ...
        azadCandidateOut.Candidate_Payment_Out_Schema.final_Status =
          final_Status;
        azadCandidateOut.Candidate_Payment_Out_Schema.trade = trade;
        azadCandidateOut.Candidate_Payment_Out_Schema.flight_Date = flight_Date
          ? flight_Date
          : "Not Fly";
      ...
      for (const agent of agents) {
        if (agent.payment_In_Schema && agent.payment_In_Schema.persons) {
          const personIn = agent.payment_In_Schema.persons.find(
            (person) =>
      ...
      for (const ticketSupplier of ticketSuppliers) {
        if (
          ticketSupplier.Supplier_Payment_In_Schema &&
          ticketSupplier.Supplier_Payment_In_Schema.persons
        ) {
          const SupPersonIn =
            ticketSupplier.Supplier_Payment_In_Schema.persons.find(
              (person) =>
        ...
        if (
          ticketSupplier.Supplier_Payment_Out_Schema &&
          ticketSupplier.Supplier_Payment_Out_Schema.persons
        ) {
          const SupPersonOut =
            ticketSupplier.Supplier_Payment_Out_Schema.persons.find(
              (person) =>
                person.name.toString === name.toString() &&
                person.pp_No.toString() === pp_No.toString() &&
                person.entry_Mode.toString() === entryMode.toString()
        ...
        if (
          ticketSupplier.Agent_Payment_In_Schema &&
          ticketSupplier.Agent_Payment_In_Schema.persons
        ) {
          const AgentPersonIn =
            ticketSupplier.Agent_Payment_In_Schema.persons.find(
              (person) =>
                person.name.toString === name.toString() &&
                person.pp_No.toString() === pp_No.toString() &&
                person.entry_Mode.toString() === entryMode.toString()
        ...
        if (
          ticketSupplier.Agent_Payment_Out_Schema &&
          ticketSupplier.Agent_Payment_Out_Schema.persons
        ) {
          const AgentPersonOut =
            ticketSupplier.Agent_Payment_Out_Schema.persons.find(
              (person) =>
                person.name.toString === name.toString() &&
                person.pp_No.toString() === pp_No.toString() &&
                person.entry_Mode.toString() === entryMode.toString()
      ...
      for (const visitSupplier of visitSuppliers) {
        if (
          visitSupplier.Supplier_Payment_In_Schema &&
          visitSupplier.Supplier_Payment_In_Schema.persons
        ) {
          const SupPersonIn =
            visitSupplier.Supplier_Payment_In_Schema.persons.find(
              (person) =>
                person.name.toString === name.toString() &&
                person.pp_No.toString() === pp_No.toString() &&
                person.entry_Mode.toString() === entryMode.toString()
        ...
        if (
          visitSupplier.Supplier_Payment_Out_Schema &&
          visitSupplier.Supplier_Payment_Out_Schema.persons
        ) {
          const SupPersonOut =
            visitSupplier.Supplier_Payment_Out_Schema.persons.find(
              (person) =>
                person.name.toString === name.toString() &&
                person.pp_No.toString() === pp_No.toString() &&
                person.entry_Mode.toString() === entryMode.toString()
        ...
        if (
          visitSupplier.Agent_Payment_In_Schema &&
          visitSupplier.Agent_Payment_In_Schema.persons
        ) {
          const AgentPersonIn =
            visitSupplier.Agent_Payment_In_Schema.persons.find(
              (person) =>
                person.name.toString === name.toString() &&
                person.pp_No.toString() === pp_No.toString() &&
                person.entry_Mode.toString() === entryMode.toString()
        ...
        if (
          visitSupplier.Agent_Payment_Out_Schema &&
          visitSupplier.Agent_Payment_Out_Schema.persons
        ) {
          const AgentPersonOut =
            visitSupplier.Agent_Payment_Out_Schema.persons.find(
              (person) =>
                person.name.toString === name.toString() &&
                person.pp_No.toString() === pp_No.toString() &&
                person.entry_Mode.toString() === entryMode.toString()
      ...
      for (const azadSupplier of azadSuppliers) {
        if (
          azadSupplier.Supplier_Payment_In_Schema &&
          azadSupplier.Supplier_Payment_In_Schema.persons
        ) {
          const SupPersonIn =
            azadSupplier.Supplier_Payment_In_Schema.persons.find(
              (person) =>
                person.name.toString === name.toString() &&
                person.pp_No.toString() === pp_No.toString() &&
                person.entry_Mode.toString() === entryMode.toString()
        ...
        if (
          azadSupplier.Supplier_Payment_Out_Schema &&
          azadSupplier.Supplier_Payment_Out_Schema.persons
        ) {
          const SupPersonOut =
            azadSupplier.Supplier_Payment_Out_Schema.persons.find(
              (person) =>
                person.name.toString === name.toString() &&
                person.pp_No.toString() === pp_No.toString() &&
                person.entry_Mode.toString() === entryMode.toString()
        ...
        if (
          azadSupplier.Agent_Payment_In_Schema &&
          azadSupplier.Agent_Payment_In_Schema.persons
        ) {
          const AgentPersonIn =
            azadSupplier.Agent_Payment_In_Schema.persons.find(
              (person) =>
                person.name.toString === name.toString() &&
                person.pp_No.toString() === pp_No.toString() &&
                person.entry_Mode.toString() === entryMode.toString()
        ...
        if (
          azadSupplier.Agent_Payment_Out_Schema &&
          azadSupplier.Agent_Payment_Out_Schema.persons
        ) {
          const AgentPersonOut =
            azadSupplier.Agent_Payment_Out_Schema.persons.find(
              (person) =>
                person.name.toString === name.toString() &&
                person.pp_No.toString() === pp_No.toString() &&
                person.entry_Mode.toString() === entryMode.toString()
      ...
      if (ticketCandidateIn) {
        ...
        ticketCandidateIn.Candidate_Payment_In_Schema.final_Status =
          final_Status;
        ticketCandidateIn.Candidate_Payment_In_Schema.trade = trade;
        ticketCandidateIn.Candidate_Payment_In_Schema.flight_Date = flight_Date
          ? flight_Date
          : "Not Fly";
      ...
      if (ticketCandidateOut) {
        ...
        ticketCandidateOut.Candidate_Payment_Out_Schema.final_Status =
          final_Status;
        ticketCandidateOut.Candidate_Payment_Out_Schema.trade = trade;
        ticketCandidateOut.Candidate_Payment_Out_Schema.flight_Date =
          flight_Date ? flight_Date : "Not Fly";
      ...
      if (visitCandidateIn) {
        ...
        visitCandidateIn.Candidate_Payment_In_Schema.final_Status =
          final_Status;
        visitCandidateIn.Candidate_Payment_In_Schema.trade = trade;
        visitCandidateIn.Candidate_Payment_In_Schema.flight_Date = flight_Date
          ? flight_Date
          : "Not Fly";
      ...
      if (visitCandidateOut) {
        ...
        visitCandidateOut.Candidate_Payment_Out_Schema.final_Status =
          final_Status;
        visitCandidateOut.Candidate_Payment_Out_Schema.trade = trade;
        visitCandidateOut.Candidate_Payment_Out_Schema.flight_Date = flight_Date
          ? flight_Date
          : "Not Fly";
...
const getAzadCandAllPaymentsIn = async (req, res) => {
  try {
    ...
    if (user.role === "Admin") {
      const supplierPayments = await AzadCandidate.find({}).sort({
        createdAt: -1,
      });
    ...
    try {
      ...
      await existingSupplier.updateOne({
        $inc: {
          ...
          "Candidate_Payment_Out_Schema.total_Payment_Out_Curr": curr_Amount
            ? -curr_Amount
            : 0,
          "Candidate_Payment_Out_Schema.remaining_Curr": curr_Amount
            ? curr_Amount
            : 0,
        },
...
const updateAzadCandSinglePaymentOut = async (req, res) => {
  ...
  if (user && user.role === "Admin") {
    ...
    try {
      ...
      // Find the payment within the payment array using paymentId
      const paymentToUpdate =
        existingSupplier.Candidate_Payment_Out_Schema.payment.find(
          (payment) => payment._id.toString() === paymentId
        );
      ...
      await existingSupplier.updateOne({
        $inc: {
          ...
          "Candidate_Payment_Out_Schema.remaining_Curr": updateCurr_Amount
            ? -updateCurr_Amount
            : 0,
        },
      });
      ...
      for (const payment of multiplePayment) {
        ...
        for (const supplier of suppliers) {
          if (supplier.Candidate_Payment_Out_Schema) {
            if (
              supplier.Candidate_Payment_Out_Schema.supplierName.toLowerCase() ===
              supplierName.toLowerCase()
            ) {
              existingSupplier = supplier;
              break;
        ...
        await existingSupplier.updateOne({
          $inc: {
            "Candidate_Payment_Out_Schema.total_Payment_Out": payment_Out,
            "Candidate_Payment_Out_Schema.remaining_Balance": -payment_Out,
            "Candidate_Payment_Out_Schema.total_Payment_Out_Curr": newCurrAmount
              ? newCurrAmount
              : 0,
            "Candidate_Payment_Out_Schema.remaining_Curr": newCurrAmount
              ? -newCurrAmount
...
const deleteAzadCandPaymentOutSchema = async (req, res) => {
  ...
  try {
    ...
    cashInHandUpdate.$inc.total_Cash =
      existingSupplier.Candidate_Payment_Out_Schema.total_Payment_Out;
    cashInHandUpdate.$inc.total_Cash =
      -existingSupplier.Candidate_Payment_Out_Schema.total_Cash_Out;
  ...
  if (user && user.role === "Admin") {
    try {
      ...
      if (azadCandidateIn) {
        ...
        azadCandidateIn.Candidate_Payment_Out_Schema.final_Status =
          final_Status;
        azadCandidateIn.Candidate_Payment_Out_Schema.trade = trade;
        azadCandidateIn.Candidate_Payment_Out_Schema.status = status;
        azadCandidateIn.Candidate_Payment_Out_Schema.flight_Date = flight_Date
          ? flight_Date
          : "Not Fly";
        await azadCandidateIn.save();
      }
      ...
      if (azadCandidateOut) {
        ...
        azadCandidateOut.Candidate_Payment_In_Schema.final_Status =
          final_Status;
        azadCandidateOut.Candidate_Payment_In_Schema.trade = trade;
        azadCandidateOut.Candidate_Payment_In_Schema.flight_Date = flight_Date
          ? flight_Date
          : "Not Fly";
        await azadCandidateOut.save();
      }
...
const getAzadCandAllPaymentsOut = async (req, res) => {
  try {
    ...
    if (user.role === "Admin") {
      const supplierPayments = await AzadAzadCandidate.find({}).sort({
        createdAt: -1,
      });
      const formattedDetails = supplierPayments
