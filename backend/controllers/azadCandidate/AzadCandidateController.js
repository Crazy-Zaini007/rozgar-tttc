...
const candidateIn=await AzadCandidate.findOne({
  ...
  const candidateOut=await AzadCandidate.findOne({
       ...
       for(const azadSupplier of azadSuppliers){
        if(azadSupplier.payment_In_Schema && azadSupplier.payment_In_Schema.persons){
          ...
          if(SupPersonIn){
            ...
            SupPersonIn.final_Status=final_Status
            SupPersonIn.trade=trade
            SupPersonIn.azad_Visa_Price_In_PKR=visa_Price
            SupPersonIn.remaining_Price-=newRemainingPKR
            SupPersonIn.azad_Visa_Price_In_Curr=visa_Price_Curr
            SupPersonIn.remaining_Curr-=newRemainingCurr
            SupPersonIn.flight_Date=flight_Date?flight_Date:'Not Fly'
       ...
       for(const azadAgent of azadAgents){
        if(azadAgent.payment_In_Schema && azadAgent.payment_In_Schema.persons){
          ...
          if(AgentPersonIn){
            ...
            AgentPersonIn.final_Status=final_Status
            AgentPersonIn.trade=trade
            AgentPersonIn.azad_Visa_Price_In_PKR=visa_Price
            AgentPersonIn.remaining_Price-=newRemainingPKR
            AgentPersonIn.azad_Visa_Price_In_Curr=visa_Price_Curr
            AgentPersonIn.remaining_Curr-=newRemainingCurr
            AgentPersonIn.flight_Date=flight_Date?flight_Date:'Not Fly'
...
if(entry){
  ...
  entry.trade=trade
  entry.azad_Visa_Sales_PKR=visa_Price
  entry.azad_Visa_Sales_Rate_Oth_Cur=visa_Price_Curr
...
const updateAgentTotalPaymentOut = async (req, res) => {
  ...
  if(candidateOut){
    ...
    candidateOut.payment_Out_Schema.total_Visa_Price_Out_PKR=visa_Price
    candidateOut.payment_Out_Schema.remaining_Balance-=newRemainingPKR
    candidateOut.payment_Out_Schema.total_Visa_Price_Out_Curr=visa_Price_Curr
    candidateOut.payment_Out_Schema.remaining_Curr-=newRemainingCurr
    candidateOut.payment_Out_Schema.flight_Date=flight_Date?flight_Date:'Not Fly'
    await candidateOut.save()
       ...
       for(const azadSupplier of azadSuppliers){
        ...
        if(azadSupplier.payment_Out_Schema && azadSupplier.payment_Out_Schema.persons){
          ...
          if(SupPersonOut){
            ...
            SupPersonOut.final_Status=final_Status
            SupPersonOut.trade=trade
            SupPersonOut.azad_Visa_Price_Out_PKR=visa_Price
            SupPersonOut.remaining_Price-=newRemainingPKR
            SupPersonOut.azad_Visa_Price_Out_Curr=visa_Price_Curr
            SupPersonOut.remaining_Curr-=newRemainingCurr
            SupPersonOut.flight_Date=flight_Date?flight_Date:'Not Fly'
           await azadSupplier.save()
...
for (const azadAgent of azadAgents){
  ...
  if(azadAgent.payment_Out_Schema && azadAgent.payment_Out_Schema.persons){
    ...
    if(AgentPersonOut){
      ...
      AgentPersonOut.final_Status=final_Status
      AgentPersonOut.trade=trade
      AgentPersonOut.azad_Visa_Price_Out_PKR=visa_Price
      AgentPersonOut.remaining_Price-=newRemainingPKR
      AgentPersonOut.azad_Visa_Price_Out_Curr=visa_Price_Curr
      AgentPersonOut.remaining_Curr-=newRemainingCurr
      AgentPersonOut.flight_Date=flight_Date?flight_Date:'Not Fly'
     await azadAgent.save()
   ...
   for(const protector of protectors){
    if(protector.payment_Out_Schema && protector.payment_Out_Schema.persons){
      ...
      if(personOut){
       ...
       personOut.final_Status=final_Status
       personOut.trade=trade
       entry.azad_Visa_Purchase_PKR=visa_Price
       entry.azad_Visa_Purchase_Rate_Oth_Cur=visa_Price_Curr
       personOut.flight_Date=flight_Date?flight_Date:'Not Fly'
       await protector.save()
