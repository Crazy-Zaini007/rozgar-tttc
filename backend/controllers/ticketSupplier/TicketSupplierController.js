    ...
    try {
        ...
        if (user) {
            ...
            if (user.role === "Admin") {
                ...
                const existingSupplier = await TicketSuppliers.findOne({ 'Supplier_Payment_In_Schema.supplierName': supplierName });
                if (!existingSupplier) {
                    res.status(404).json({
                        message: "Supplier not Found"
                    });
...
const updateSupPaymentInPerson=async(req,res)=>{
  ...
  if(candidateOut){
    ...
    await candidateOut.save()
         ...
         const azadSuppliers=await TicketSuppliers.find({})
...
const getAllAzadSupplierPaymentsIn = async (req, res) => {
    try {
        ...
        if (user.role === "Admin") {
            const supplierPayments = await TicketSuppliers.find({}).sort({ createdAt: -1 });
    ...
    try {
        ...
        if (user) {
            ...
            if (user.role === "Admin") {
                ...
                const existingSupplier = await TicketSuppliers.findOne({ 'Agent_Payment_In_Schema.supplierName': supplierName });
                if (!existingSupplier) {
                    res.status(404).json({
                        message: "Supplier not Found"
                    });
