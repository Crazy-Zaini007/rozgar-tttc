  ...
  if (user && user.role === "Admin") {
    try {
      ...
      if (entry) {
        ...
        entry.trade = trade;
        entry.ticket_Sales_PKR=new_Visa_Price_PKR
        entry.ticket_Sales_Rate_Oth_Cur=new_Visa_Price_Cur
  ...
  if (user && user.role === "Admin") {
    try {
      ...
      if (entry) {
        ...
        entry.final_Status = final_Status;
        entry.trade = trade;
        entry.ticket_Purchase_PKR=new_Visa_Price_PKR
        entry.ticket_Purchase_Rate_Oth_Cur=new_Visa_Price_Cur
        entry.flight_Date = flight_Date ? flight_Date : "Not Fly";
        await entry.save();
      }
...
const changeSupplierPaymentInStatus = async (req, res) => {
  try {
    ...
    if (existingSupplier.payment_In_Schema.status === "Open") {
      ...
      responseMessage =
        "Ticket Supplier Status updated to Closed Successfully!";
      const newNotification = new Notifications({
...
const addCandVisePaymentIn = async (req, res) => {
  try {
    ...
    const newNotification = new Notifications({
      ...
      content: `${user.userName} added Candidate Wise Payment_In: ${new_Payment_In} to ${payments.length} Candidates of Ticket Supplier:${supplierName}`,
      date: new Date().toISOString().split("T")[0],
    });
    await newNotification.save();
    ...
    await existingSupplier.save();
    res.status(200).json({
      message: `Payment In: ${new_Payment_In} added Successfully for to ${payments.length} Candidates to Ticket Supplier:${supplierName}'s Record`,
    });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
    ...
    if (paymentToFind) {
      ...
      if (candPayment) {
        ...
        const newNotification = new Notifications({
          ...
          content: `${user.userName} deleted Cand-Wise Payment_In: ${candPayment.new_Payment} of Candidate ${candPayment.cand_Name} from  Ticket Supplier:${supplierName}'s Record`,
          date: new Date().toISOString().split("T")[0],
        });
        await newNotification.save();
        ...
        res.status(200).json({
          message: `Successfuly, deleted Cand-Wise Payment_In: ${candPayment.new_Payment} of Candidate ${candPayment.cand_Name} from  Ticket Supplier:${supplierName}'s Record`,
        });
      }
    }
  } catch (error) {
    ...
    if (paymentToFind) {
      ...
      if (candPayment) {
        ...
        const newNotification = new Notifications({
          ...
          content: `${user.userName} updated Cand-Wise Payment_In ${new_Payment} of Candidate ${candPayment.cand_Name} of Ticket Supplier:${supplierName}'s Record`,
          date: new Date().toISOString().split("T")[0],
        });
        await newNotification.save();
        await existingSupplier.save();
        ...
        res.status(200).json({
          message: `Successfuly, updated Cand-Wise Payment_In ${new_Payment} of Candidate ${candPayment.cand_Name} of Ticket Supplier: ${supplierName}'s Record`,
        });
      }
    }
  } catch (error) {
...
const addCandVisePaymentOut = async (req, res) => {
  try {
    ...
    const newNotification = new Notifications({
      ...
      content: `${user.userName} added Candidate Wise Payment_Ou: ${new_Payment_Out} to ${payments.length} Candidates of Ticket Supplier:${supplierName}`,
      date: new Date().toISOString().split("T")[0],
    });
    await newNotification.save();
    ...
    await existingSupplier.save();
    res.status(200).json({
      message: `Payment Out: ${new_Payment_Out} added Successfully for to ${payments.length} Candidates to Ticket Supplier:${supplierName}'s Record`,
    });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
  ...
  try {
    ...
    if (paymentToDelete) {
      ...
      const newNotification = new Notifications({
        ...
        content: `${user.userName} deleted Cand-Wise Payment_Out: ${paymentToDelete.payment_Out} of ${paymentToDelete.payments.length} Candidates from  Ticket Supplier:${supplierName}'s Record`,
        date: new Date().toISOString().split("T")[0],
      });
      await newNotification.save();

      res.status(200).json({
        message: `Payment Out with ID ${paymentId} deleted successfully of ${paymentToDelete.payments.length} Candidates from  Ticket Supplier:${supplierName}'s Record`,
      });
    }
  } catch (error) {
    res.status(500).json({ message: error.message });
  ...
  try {
    ...
    if (paymentToFind) {
      ...
      if (candPayment) {
        ...
        const newNotification = new Notifications({
          type: "Ticket Supplier Cand-Wise Payment Out Deleted",
          content: `${user.userName} deleted Cand-Wise Payment_Out: ${candPayment.new_Payment} of Candidate ${candPayment.cand_Name} from  Ticket Supplier:${supplierName}'s Record`,
          date: new Date().toISOString().split("T")[0],
        });
        await newNotification.save();
        const candPaymentToDelete = paymentToFind.payments.filter(
        ...
        res.status(200).json({
          message: `Successfuly, deleted Cand-Wise Payment_Out: ${candPayment.new_Payment} of Candidate ${candPayment.cand_Name} from  Ticket Supplier:${supplierName}'s Record`,
        });
      }
    }
  } catch (error) {
    ...
    if (paymentToFind) {
      ...
      if (candPayment) {
        ...
        const newNotification = new Notifications({
          ...
          content: `${user.userName} updated Cand-Wise Payment_Out: ${new_Payment} of Candidate ${candPayment.cand_Name} of Ticket Supplier:${supplierName}'s Record`,
          date: new Date().toISOString().split("T")[0],
        });
        await newNotification.save();
        await existingSupplier.save();
        ...
        res.status(200).json({
          message: `Successfuly, updated Cand-Wise Payment_Out ${new_Payment} of Candidate ${candPayment.cand_Name} of Ticket Supplier:${supplierName}'s Record`,
        });
      }
    }
  } catch (error) {
