  ...
  if (user && user.role === "Admin") {
    ...
    try {
      ...
      const newNotification = new Notifications({
        ...
        } of Visit Supplier: ${supplierName}`,
        date: new Date().toISOString().split("T")[0],
      });
      await newNotification.save();
      const updatedSupplier = await VisitSuppliers.findById(
...
const getAllAzadSupplierPaymentsIn = async (req, res) => {
  try {
    ...
    if (user.role === "Admin") {
      const supplierPayments = await VisitSuppliers.find({}).sort({
        createdAt: -1,
      });
...
const addCandVisePaymentIn = async (req, res) => {
  try {
    ...
    if (!existingSupplier) {
      res.status(404).json({
        message: "Visit Supplier not Found",
      });
      return;
    }
    ...
    const newNotification = new Notifications({
      type: "Visit Supplier Cand-Wise Payment In",
      content: `${user.userName} added Candidate Wise Payment_In: ${new_Payment_In} to ${payments.length} Candidates of Visit Supplier:${supplierName}`,
      date: new Date().toISOString().split("T")[0],
    });
    await newNotification.save();
    ...
    await existingSupplier.save();
    res.status(200).json({
      message: `Payment In: ${new_Payment_In} added Successfully for to ${payments.length} Candidates to Visit Supplier:${supplierName}'s Record`,
    });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
const deleteCandVisePaymentIn = async (req, res) => {
  try {
    ...
    if (paymentToDelete) {
      ...
      await existingSupplier.save();
      const newNotification = new Notifications({
        type: "Visit Suppliers Cand-Wise Payment In Deleted",
...
const updateCandVisePaymentIn = async (req, res) => {
  ...
  if (!existingSupplier) {
    res.status(404).json({ message: "Visit Supplier not found" });
    return;
  }
  ...
  try {
    ...
    if (paymentToFind) {
      ...
      if (candPayment) {
        ...
        await CashInHand.updateOne({}, cashInHandUpdate);
        const newNotification = new Notifications({
          type: "Visit Supplier Cand-Wise Payment In Deleted",
          content: `${user.userName} deleted Cand-Wise Payment_In: ${candPayment.new_Payment} of Candidate ${candPayment.cand_Name} from  Visit Supplier:${supplierName}'s Record`,
          date: new Date().toISOString().split("T")[0],
        });
        await newNotification.save();
        ...
        res.status(200).json({
          message: `Successfuly, deleted Cand-Wise Payment_In: ${candPayment.new_Payment} of Candidate ${candPayment.cand_Name} from  Visit Supplier:${supplierName}'s Record`,
        });
      }
    }
  } catch (error) {
  ...
  try {
    ...
    if (paymentToFind) {
      ...
      if (candPayment) {
        ...
        await CashInHand.updateOne({}, cashInHandUpdate);
        const newNotification = new Notifications({
          type: "Visit Supplier Cand-Wise Payment In Updated",
          content: `${user.userName} updated Cand-Wise Payment_In ${new_Payment} of Candidate ${candPayment.cand_Name} of Visit Supplier:${supplierName}'s Record`,
          date: new Date().toISOString().split("T")[0],
        });
        await newNotification.save();
        await existingSupplier.save();
        ...
        res.status(200).json({
          message: `Successfuly, updated Cand-Wise Payment_In ${new_Payment} of Candidate ${candPayment.cand_Name} of Visit Supplier: ${supplierName}'s Record`,
        });
      }
    }
  } catch (error) {
...
const addCandVisePaymentOut = async (req, res) => {
  try {
    ...
    const newNotification = new Notifications({
      type: "Visit Supplier Cand-Wise Payment Out",
      content: `${user.userName} added Candidate Wise Payment_Ou: ${new_Payment_Out} to ${payments.length} Candidates of Visit Supplier:${supplierName}`,
      date: new Date().toISOString().split("T")[0],
    });
    await newNotification.save();
    ...
    await existingSupplier.save();
    res.status(200).json({
      message: `Payment Out: ${new_Payment_Out} added Successfully for to ${payments.length} Candidates to Visit Supplier:${supplierName}'s Record`,
    });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
const deleteCandVisePaymentOut = async (req, res) => {
  try {
    ...
    if (paymentToDelete) {
      ...
      await existingSupplier.save();
      const newNotification = new Notifications({
        type: "Visit Supplier Cand-Wise Payment Out Deleted",
        content: `${user.userName} deleted Cand-Wise Payment_Out: ${paymentToDelete.payment_Out} of ${paymentToDelete.payments.length} Candidates from  Visit Supplier:${supplierName}'s Record`,
        date: new Date().toISOString().split("T")[0],
      });
      await newNotification.save();

      res.status(200).json({
        message: `Payment Out with ID ${paymentId} deleted successfully of ${paymentToDelete.payments.length} Candidates from  Visit Supplier:${supplierName}'s Record`,
      });
    }
  } catch (error) {
    res.status(500).json({ message: error.message });
  ...
  try {
    ...
    if (paymentToFind) {
      ...
      if (candPayment) {
        ...
        const newNotification = new Notifications({
          type: "Visit Supplier Cand-Wise Payment Out Deleted",
          content: `${user.userName} deleted Cand-Wise Payment_Out: ${candPayment.new_Payment} of Candidate ${candPayment.cand_Name} from  Visit Supplier:${supplierName}'s Record`,
          date: new Date().toISOString().split("T")[0],
        });
        await newNotification.save();
        const candPaymentToDelete = paymentToFind.payments.filter(
        ...
        res.status(200).json({
          message: `Successfuly, deleted Cand-Wise Payment_Out: ${candPayment.new_Payment} of Candidate ${candPayment.cand_Name} from  Visit Supplier:${supplierName}'s Record`,
        });
      }
    }
  } catch (error) {
  ...
  try {
    ...
    if (paymentToFind) {
      ...
      if (candPayment) {
        ...
        await CashInHand.updateOne({}, cashInHandUpdate);
        const newNotification = new Notifications({
          type: "Visit Supplier Cand-Wise Payment Out Updated",
          content: `${user.userName} updated Cand-Wise Payment_Out: ${new_Payment} of Candidate ${candPayment.cand_Name} of Visit Supplier:${supplierName}'s Record`,
          date: new Date().toISOString().split("T")[0],
        });
        await newNotification.save();
        await existingSupplier.save();
        ...
        res.status(200).json({
          message: `Successfuly, updated Cand-Wise Payment_Out ${new_Payment} of Candidate ${candPayment.cand_Name} of Visit Supplier:${supplierName}'s Record`,
        });
      }
    }
  } catch (error) {
