...
const addAzadCandMultiplePaymentsIn = async (req, res) => {
    try {
        ...
        try {
            ...
            res.status(200).json({
                message: `${multiplePayment.length} Payments In added Successfully to ${updatedSupplier.Candidate_Payment_In_Schema.supplierName}'s Record`
            });
        } catch (error) {
            console.error('Error updating values:', error);
            res.status(500).json({
...
const updateAgentTotalPaymentIn = async (req, res) => {
    ...
    if (user && user.role === "Admin") {
      try {
        ...
        const azadCandidateIn=await TicketCandidate.findOne({
        ...
        if(final_Status.toLowerCase()==='offer letter' || final_Status.toLowerCase()==='offer_letter'){
          ...
          const azadCandidateOut=await TicketCandidate.findOne({
