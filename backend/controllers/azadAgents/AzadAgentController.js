    ...
    try {
        ...
        if (user) {
            ...
            if (user.role === "Admin") {
                ...
                const existingSupplier = await AzadAgents.findOne({ 'payment_In_Schema.supplierName': supplierName });
                if (!existingSupplier) {
                    res.status(404).json({
                        message: "Supplier not Found"
                    });
                ...
                try {
                    ...
                    const newBackup=new Backup({
                      ...
                      invoice: nextInvoiceNumber,
                        ...
                        await newBackup.save()
                        const newNotification=new Notifications({
                          type:"Azad Agent Payment In Return",
                          content:`${user.userName} added Payment_Return: ${cash_Out}  of Azad Agent: ${supplierName}`,
                          date: new Date().toISOString().split("T")[0]
    ...
    if (user && user.role === "Admin") {
        ...
        try {
          ...
          await newRecycle.save()
            ...
            await CashInHand.updateOne({}, cashInHandUpdate);
            const newNotification=new Notifications({
              type:"Azad Agent Payment In Deleted",
              content:`${user.userName} deleted ${payment_In ? "Payment_In":"Cash_Retrun"}: ${payment_In ? payment_In :cash_Out} of Azad Agent: ${supplierName}`,
              date: new Date().toISOString().split("T")[0]
...
const updateSingleAzadAgentPaymentIn = async (req, res) => {
    ...
    if (user && user.role === "Admin") {
        ...
        try {
            ...
            await existingSupplier.save();
            const newNotification=new Notifications({
              type:"Azad Agent Payment In Updated",
              content:`${user.userName} updated Payment_In: ${payment_In} of Azad Agent: ${supplierName}`,
              date: new Date().toISOString().split("T")[0]
        ...
        try {
            ...
            await existingSupplier.updateOne({
                $inc: {
                    ...
                    'payment_In_Schema.total_Azad_Visa_Price_In_PKR': -azad_Visa_Price_In_PKR,
                    'payment_In_Schema.total_Azad_Visa_Price_In_Curr': azad_Visa_Price_In_Curr?-azad_Visa_Price_In_Curr:0,
            ...
            const newNotification=new Notifications({
              type:"Azad Agent Payment In Person Deleted",
              content:`${user.userName} deleted Person having Visa Price In PKR: ${azad_Visa_Price_In_PKR} of Azad Agent: ${supplierName}`,
              date: new Date().toISOString().split("T")[0]
  ...
  if (user ) {
    try {
      ...
      if(final_Status.toLowerCase()==='ticket' || final_Status.toLowerCase()==='tiket'){
       ...
       const agents=await AzadAgents.find({})
   ...
   await visitAgent.save()
       ...
       for(const azadSupplier of azadSuppliers){
        if(azadSupplier.payment_In_Schema && azadSupplier.payment_In_Schema.persons){
          const AgentPersonIn= azadSupplier.payment_In_Schema.persons.find(person=> person.name.toString ===name.toString() && person.pp_No.toString()===pp_No.toString() && person.entry_Mode.toString()===entryMode.toString())
          if(AgentPersonIn){
            AgentPersonIn.company=company
            AgentPersonIn.country=country
            AgentPersonIn.entry_Mode=entry_Mode
        ...
        if(azadSupplier.payment_Out_Schema && azadSupplier.payment_Out_Schema.persons){
          const AgentPersonOut= azadSupplier.payment_Out_Schema.persons.find(person=> person.name.toString ===name.toString() && person.pp_No.toString()===pp_No.toString() && person.entry_Mode.toString()===entryMode.toString())
          if(AgentPersonOut){
            AgentPersonOut.company=company
            AgentPersonOut.country=country
            AgentPersonOut.entry_Mode=entry_Mode
...
const newNotification=new Notifications({
  type:"Azad Agent Payment In Person Updated",
  content:`${user.userName} updated Person :${name} of Azad Agent: ${supplierName}`,
  date: new Date().toISOString().split("T")[0]
...
const getAllAzadAgentPaymentsIn = async (req, res) => {
    try {
        ...
        if (user.role === "Admin") {
            const supplierPayments = await AzadAgents.find({}).sort({ createdAt: -1 });
        ...
        if (user) {
            ...
            if (user.role === "Admin") {
                ...
                try {
                    ...
                    const newNotification=new Notifications({
                      type:"Azad Agent Payment Out Return",
                      content:`${user.userName} added Payment_Return: ${cash_Out} of Azad Agent: ${supplierName}`,
                      date: new Date().toISOString().split("T")[0]
...
const deleteAzadAgentSinglePaymentOut = async (req, res) => {
    ...
    if (user && user.role === "Admin") {
        ...
        await newRecycle.save()
            ...
            const newNotification=new Notifications({
              type:"Azad Agent Payment Out Deleted",
              content:`${user.userName} deleted ${payment_Out ? "Payment_Out":"Cash_Retrun"}: ${payment_Out ? payment_Out :cash_Out} of Azad Agent: ${supplierName}`,
              date: new Date().toISOString().split("T")[0]
        ...
        try {
            ...
            await existingSupplier.updateOne({
                $inc: {
                    ...
                    'payment_Out_Schema.total_Azad_Visa_Price_Out_PKR': -newVisa_Price_Out_PKR,
                    'payment_Out_Schema.total_Azad_Visa_Price_Out_Curr': -newVisa_Price_Out_Curr,
            ...
            const newNotification=new Notifications({
              type:"Azad Agent Payment Out Person Deleted",
              content:`${user.userName} deleted Person having Visa Price In PKR: ${azad_visa_Price_Out_PKR} of Azad Agent: ${supplierName}`,
              date: new Date().toISOString().split("T")[0]
...
const newNotification=new Notifications({
  type:"Azad Agent Payment Out Person Updated",
  content:`${user.userName} updated Person :${name} of Azad Agent: ${supplierName}`,
  date: new Date().toISOString().split("T")[0]
