  ...
  if (user) {
    try {
      ...
      for (const azadSupplier of azadSuppliers) {
        if (
          azadSupplier.payment_In_Schema &&
          azadSupplier.payment_In_Schema.persons
        ) {
          const AgentPersonIn = azadSupplier.payment_In_Schema.persons.find(
            (person) =>
              person.name.toString === name.toString() &&
              person.pp_No.toString() === pp_No.toString() &&
              person.entry_Mode.toString() === entryMode.toString()
        ...
        if (
          azadSupplier.payment_Out_Schema &&
          azadSupplier.payment_Out_Schema.persons
        ) {
          const AgentPersonOut = azadSupplier.payment_Out_Schema.persons.find(
            (person) =>
              person.name.toString === name.toString() &&
              person.pp_No.toString() === pp_No.toString() &&
              person.entry_Mode.toString() === entryMode.toString()
...
const getAllAzadAgentPaymentsIn = async (req, res) => {
  try {
    ...
    if (user.role === "Admin") {
      const supplierPayments = await AzadAgents.find({}).sort({
        createdAt: -1,
      });
...
const addAgentCandVisePaymentIn = async (req, res) => {
  try {
    ...
    const newNotification = new Notifications({
      type: "Azad Agent Cand-Wise Payment In",
...
const deleteAgentCandVisePaymentIn = async (req, res) => {
  try {
    ...
    if (paymentToDelete) {
      ...
      await existingSupplier.save();
      const newNotification = new Notifications({
        type: "Azad Agents Cand-Wise Payment In Deleted",
...
const updateAgentCandVisePaymentIn = async (req, res) => {
  ...
  if (!existingSupplier) {
    res.status(404).json({ message: "Azad Agent not found" });
    return;
  }
  ...
  try {
    ...
    if (paymentToFind) {
      ...
      if (candPayment) {
        ...
        await CashInHand.updateOne({}, cashInHandUpdate);
        const newNotification = new Notifications({
          type: "Azad Agent Cand-Wise Payment In Deleted",
  ...
  try {
    ...
    if (paymentToFind) {
      ...
      if (candPayment) {
        ...
        await CashInHand.updateOne({}, cashInHandUpdate);
        const newNotification = new Notifications({
          type: "Azad Agent Cand-Wise Payment In Updated",
...
const addAgentCandVisePaymentOut = async (req, res) => {
  try {
    ...
    const newNotification = new Notifications({
      type: "Azad Agent Cand-Wise Payment Out",
...
const deleteAgentCandVisePaymentOut = async (req, res) => {
  try {
    ...
    if (paymentToDelete) {
      ...
      await existingSupplier.save();
      const newNotification = new Notifications({
        type: "Azad Agent Cand-Wise Payment Out Deleted",
  ...
  try {
    ...
    if (paymentToFind) {
      ...
      if (candPayment) {
        ...
        await CashInHand.updateOne({}, cashInHandUpdate);
        const newNotification = new Notifications({
          type: "Azad Agent Cand-Wise Payment Out Updated",
...
module.exports = {
  addAzadAgentPaymentIn,
  addAzadAgentMultiplePaymentsIn,
  addAzadAgentPaymentInReturn,
  deleteSingleAgentPaymentIn,
  updateSingleAzadAgentPaymentIn,
  deleteAzadAgentPaymentInPerson,
  updateAgentPaymentInPerson,
  deleteAzadAgentPaymentInSchema,
  getAllAzadAgentPaymentsIn,
  addAzadAgentPaymentOut,
  addAzadAgentMultiplePaymentsOut,
  addAzadAgentPaymentOutReturn,
  deleteAzadAgentSinglePaymentOut,
  updateAzadAgentSinglePaymentOut,
  deleteAzadAgentPaymentOutPerson,
  updateAgentPaymentOutPerson,
  deleteAzadAgentPaymentOutSchema,
  getAllAzadAgentPaymentsOut,
  changeAgentPaymentInStatus,
  changeAgentPaymentOutStatus,
  addAgentCandVisePaymentIn,
  ...
  updateSingleAgentCandVisePaymentOut,
};
