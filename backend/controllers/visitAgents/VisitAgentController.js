...
const updateAgentPaymentInPerson = async (req, res) => {
  ...
  if (user) {
    try {
      ...
      const agents = await VisitAgents.find({});
      ...
      const visitAgents = await AzadAgents.find({});
      ...
      if (entry) {
        ...
        entry.visit_Sales_PKR=new_Visa_Price_PKR
        entry.visit_Sales_Rate_Oth_Curr=new_Visa_Price_Cur
...
const getAllAzadAgentPaymentsIn = async (req, res) => {
  try {
    ...
    if (user.role === "Admin") {
      const supplierPayments = await VisitAgents.find({}).sort({
        createdAt: -1,
      });
  ...
  if (user && user.role === "Admin") {
    try {
      ...
      if (entry) {
        ...
        entry.visit_Purchase_Rate_PKR=new_Visa_Price_PKR
        entry.visit_Purchase_Rate_Oth_Cur=new_Visa_Price_Cur
...
const changeAgentPaymentInStatus = async (req, res) => {
  try {
    ...
    const newSupplier=new VisitAgents({
      payment_In_Schema:{
    ...
    let responseMessage;
        ...
        const newNotification=new Notifications({
          ...
          content:`${user.userName} Closed Khata with Visit Agent:${supplierName} and new Khata created successfully`,
          date: new Date().toISOString().split("T")[0]
