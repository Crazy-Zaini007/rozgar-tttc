    ...
    try {
        ...
        if (user) {
            ...
            if (user.role === "Admin") {
                ...
                const existingSupplier = await VisitAgents.findOne({ 'payment_In_Schema.supplierName': supplierName });
                if (!existingSupplier) {
                    res.status(404).json({
                        message: "Supplier not Found"
                    });
    ...
    if (user && user.role === "Admin") {
        ...
        try {
          ...
          await newRecycle.save()
            ...
            await newNotification.save()
            // const updatedSupplier = await VisitAgents.findById(existingSupplier._id);
...
const getAllAzadAgentPaymentsIn = async (req, res) => {
    try {
        ...
        if (user.role === "Admin") {
            const supplierPayments = await VisitAgents.find({}).sort({ createdAt: -1 });
